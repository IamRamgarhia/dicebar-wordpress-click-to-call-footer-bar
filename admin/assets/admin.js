...
( function () {
	...
	var config = window.dicebarAdmin;
	...
	var list = document.getElementById( 'dicebar-items' );
	var addButton = document.getElementById( 'dicebar-add-item' );
	var hint = document.getElementById( 'dicebar-add-hint' );
	var empty = document.querySelector( '.dicebarui__empty' );
	var activeTab = document.getElementById( 'dicebar-active-tab' );
	...
	function field( label, control, role ) {
		return make( 'label', { class: 'dicebarui__field', 'data-role': role || null }, [
	...
	function iconPicker( name, current ) {
		var grid = make( 'div', { class: 'dicebarui__iconpicker', role: 'radiogroup' } );
		...
		config.icons.forEach( function ( icon ) {
			...
			svg.setAttribute( 'class', 'dicebarui__iconglyph' );
			svg.setAttribute( 'viewBox', '0 0 24 24' );
			svg.setAttribute( 'aria-hidden', 'true' );
			use.setAttribute( 'href', '#dicebar-i-' + icon );
			svg.appendChild( use );

			grid.appendChild(
				make( 'label', { class: 'dicebarui__iconopt', title: icon }, [
					input,
					svg,
	...
	function buildRow( index ) {
		...
		var base = 'dicebar[items][' + index + ']';

		var head = make( 'div', { class: 'dicebarui__item-head' }, [
			make( 'span', { class: 'dicebarui__grip', 'aria-hidden': 'true' } ),
			make( 'strong', { class: 'dicebarui__item-title', text: strings.untitled } ),
			make( 'code', { class: 'dicebarui__item-id', text: strings.newId } ),
			make( 'button', {
				type: 'button',
				class: 'dicebarui__remove',
				text: strings.remove,
			} ),
		] );

		var fields = make( 'div', { class: 'dicebarui__fields' }, [
			...
			make( 'div', { class: 'dicebarui__field dicebarui__field--icons', 'data-role': 'iconfield' }, [
				make( 'span', { text: strings.icon } ),
				iconPicker( base + '[icon]', type.icon ),
			] ),
			make( 'div', { class: 'dicebarui__extra', 'data-role': 'extra' } ),
			make( 'label', { class: 'dicebarui__field dicebarui__field--check' }, [
		...
		var row = make( 'div', { class: 'dicebarui__item', draggable: 'true' }, [
			head,
	...
	function buildExtra( row, type, index ) {
		...
		type.extra.forEach( function ( extra ) {
			var name = 'dicebar[items][' + index + '][extra][' + extra.key + ']';
	 ...
	 */
	function reindex() {
		var rows = list.querySelectorAll( '.dicebarui__item' );

		Array.prototype.forEach.call( rows, function ( row, index ) {
			row.setAttribute( 'data-index', index );

			Array.prototype.forEach.call( row.querySelectorAll( '[name^="dicebar[items]"]' ), function ( input ) {
				input.name = input.name.replace( /dicebar\[items\]\[\d+\]/, 'dicebar[items][' + index + ']' );
			} );
		} );

		updateState();

		if ( typeof window.dicebarRefreshPreview === 'function' ) {
			window.dicebarRefreshPreview();
		}
	}
	...
	function updateState() {
		var count = list.querySelectorAll( '.dicebarui__item' ).length;
	...
	function refreshTitle( row ) {
		var title = row.querySelector( '.dicebarui__item-title' );
	...
	list.addEventListener( 'click', function ( event ) {
		if ( ! event.target.classList.contains( 'dicebarui__remove' ) ) {
			return;
		}
		...
		event.target.closest( '.dicebarui__item' ).remove();
		reindex();
	} );

	list.addEventListener( 'change', function ( event ) {
		var row = event.target.closest( '.dicebarui__item' );
	...
	list.addEventListener( 'input', function ( event ) {
		if ( event.target.name && event.target.name.indexOf( '[label]' ) !== -1 ) {
			refreshTitle( event.target.closest( '.dicebarui__item' ) );
		}
	} );
	...
	Array.prototype.forEach.call( list.querySelectorAll( '.dicebarui__item' ), function ( row ) {
		makeDraggable( row );
	...
	Array.prototype.forEach.call( document.querySelectorAll( '.dicebarui__kit' ), function ( button ) {
		button.addEventListener( 'click', function () {
			...
			if ( list.querySelector( '.dicebarui__item' ) && ! window.confirm( strings.replace ) ) {
				return;
			}
	...
	Array.prototype.forEach.call( document.querySelectorAll( '[data-advanced]' ), function ( group ) {
		var rows = group.querySelectorAll( '.dicebarui__row' );
		...
		toggle.type = 'button';
		toggle.className = 'dicebarui__advanced-toggle';
	...
	var tabs = document.querySelectorAll( '.dicebarui__tab' );
	var panels = document.querySelectorAll( '.dicebarui__panel' );
	...
	function showTab( name ) {
		...
		Array.prototype.forEach.call( panels, function ( panel ) {
			panel.hidden = panel.id !== 'dicebar-panel-' + name;
		} );
	...
	var preview = document.getElementById( 'dicebar-preview' );
	var previewInner = document.getElementById( 'dicebar-preview-inner' );
	var previewScreen = document.getElementById( 'dicebar-preview-screen' );
	var previewNote = document.getElementById( 'dicebar-preview-note' );
	var previewStage = 'photo';
	var form = document.querySelector( '.dicebarui__form' );
	...
	function refreshPreview() {
		...
		var mode = chosen( 'dicebar[style][label][mode]', 'icon_label' );
		var look = chosen( 'dicebar[style][preset]', 'glass' );

		preview.className = [
			'dicebar',
			'dicebar--preview',
			previewStage === 'dark' ? 'dicebar--scheme-dark' : 'dicebar--scheme-light',
			'dicebar--preset-' + look,
			'dicebar--item-' + value( 'dicebar[style][item][shape]', 'plain' ),
			'dicebar--shadow-' + value( 'dicebar[style][shadow]', 'soft' ),
			'dicebar--show-' + mode.replace( '_', '-' ),
			'dicebar--divider-' + ( value( 'dicebar[style][divider]', '' ) ? 'hairline' : 'none' ),
			'dicebar--case-upper',
			value( 'dicebar[style][blur]', '' ) ? 'dicebar--blur' : 'dicebar--no-blur',
		].join( ' ' );

		var palette = previewStage === 'dark' ? 'dark' : 'light';

		var tokens = {
			'--dicebar-bar-bg': value( 'dicebar[style][' + palette + '][bar_bg]', previewStage === 'dark' ? '#1c1c1e' : '#ffffff' ),
			'--dicebar-text': value( 'dicebar[style][' + palette + '][text]', '#1c1c1e' ),
			'--dicebar-icon': value( 'dicebar[style][' + palette + '][icon]', '#1c1c1e' ),
			'--dicebar-accent': value( 'dicebar[style][' + palette + '][accent]', '#0a84ff' ),
			'--dicebar-hover-bg': value( 'dicebar[style][' + palette + '][hover_bg]', '#0a84ff' ),
			'--dicebar-hover-text': value( 'dicebar[style][' + palette + '][hover_text]', '#ffffff' ),
			'--dicebar-opacity': value( 'dicebar[style][opacity]', '78' ) + '%',
			'--dicebar-glass': value( 'dicebar[style][glass]', '22' ) + 'px',
			'--dicebar-radius': value( 'dicebar[style][radius]', '18' ) + 'px',
		};
		...
		if ( ! rows.length ) {
			...
			empty.className = 'dicebarui__preview-empty';
		...
		Array.prototype.forEach.call( rows, function ( row ) {
			...
			var brand = value( 'dicebar[style][brand_icons]', '' ) && config.brandColors[ icon ]
				? config.brandColors[ icon ]
				: '';

			button.className = 'dicebar__item'
				+ ( primary && primary.checked ? ' dicebar__item--primary' : '' )
				+ ( brand ? ' dicebar__item--brand' : '' );

			if ( brand ) {
				button.style.setProperty( '--dicebar-brand', brand );
			}
			...
			if ( mode !== 'label' && icon ) {
				...
				svg.setAttribute( 'class', 'dicebar__icon' );
				svg.setAttribute( 'viewBox', '0 0 24 24' );
				svg.setAttribute( 'aria-hidden', 'true' );
				use.setAttribute( 'href', '#dicebar-i-' + icon );
				svg.appendChild( use );
	...
	if ( preview && form ) {
		...
		Array.prototype.forEach.call( document.querySelectorAll( '.dicebarui__preview-stages button' ), function ( button ) {
			button.addEventListener( 'click', function () {
		...
		Array.prototype.forEach.call( document.querySelectorAll( '.dicebarui__preview-widths button' ), function ( button ) {
			button.addEventListener( 'click', function () {
				Array.prototype.forEach.call( button.parentNode.children, function ( other ) {
					other.classList.toggle( 'is-current', other === button );
				} );
		...
		window.dicebarRefreshPreview = refreshPreview;
		refreshPreview();
	}
