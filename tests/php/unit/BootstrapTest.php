...
class BootstrapTest extends WP_UnitTestCase {
	...
	public function test_constants_are_defined() {
		$this->assertTrue( defined( 'DICEBAR_VERSION' ) );
		$this->assertTrue( defined( 'DICEBAR_MIN_PHP' ) );
		$this->assertTrue( defined( 'DICEBAR_FILE' ) );
		$this->assertTrue( defined( 'DICEBAR_PATH' ) );
		$this->assertTrue( defined( 'DICEBAR_URL' ) );
		$this->assertSame( 1, DICEBAR_SCHEMA_VERSION );
	}

	/**
	 * DICEBAR_PATH ends with a trailing slash and points at the plugin file.
	 */
	public function test_path_constant_has_trailing_slash() {
		$this->assertStringEndsWith( '/', DICEBAR_PATH );
		$this->assertFileExists( DICEBAR_PATH . 'dicebar.php' );
	}
	...
	public function test_php_guard_rejects_versions_below_the_floor() {
		$this->assertFalse( dicebar_php_is_supported( '7.2.0' ) );
		$this->assertFalse( dicebar_php_is_supported( '7.3.33' ) );
	}
	...
	public function test_php_guard_accepts_the_floor_and_above() {
		$this->assertTrue( dicebar_php_is_supported( '7.4.0' ) );
		$this->assertTrue( dicebar_php_is_supported( '8.3.0' ) );
	}

	/**
	 * The plugin header Version matches the DICEBAR_VERSION constant.
	 */
	public function test_version_header_matches_the_version_constant() {
		$data = get_plugin_data( DICEBAR_FILE, false, false );
		$this->assertSame( DICEBAR_VERSION, $data['Version'] );
	}

	/**
	 * The readme's Stable tag matches the DICEBAR_VERSION constant.
	 */
	public function test_readme_stable_tag_matches_the_version_constant() {
		$readme = file_get_contents( DICEBAR_PATH . 'readme.txt' );
		$this->assertSame( 1, preg_match( '/^Stable tag:\s*(\S+)/m', $readme, $m ) );
		$this->assertSame( DICEBAR_VERSION, $m[1] );
	}
	...
	public function test_text_domain_header_matches_the_slug() {
		$data = get_plugin_data( DICEBAR_FILE, false, false );
		$this->assertSame( 'dicebar', $data['TextDomain'] );
	}
}
